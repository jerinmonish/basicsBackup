...
import { StudyLevelList } from './../../../actions/configuration'
...
const StudyLevel = () => {
  ...
  const StudyLevelData = useSelector((state) => state.configurationBackend)
  ...
  useEffect(() => {
    setLoading(true)
    dispatch(StudyLevelList(params))
  }, [query])

  //useEffect to set Group list data and to update data for edit privielage
  useEffect(() => {
    if (StudyLevelData?.studylevelListDetails?.data?.result.length > 0) {
      setItems(StudyLevelData?.studylevelListDetails?.data?.result)
      setPages(StudyLevelData?.studylevelListDetails?.data?.total_pages)
      setItemsPerPage(itemsPerPage)
      setLoading(false)
      setCtFrom(StudyLevelData?.studylevelListDetails?.data?.from_count);
      setCtTo(StudyLevelData?.studylevelListDetails?.data?.to_count);
      setTotalCnt(StudyLevelData?.studylevelListDetails?.data?.total_count);
      // console.log(params)
    }
    else {
      setItems([])
  ...
  }, [StudyLevelData?.studylevelListDetails?.data?.result])
              ...
              <CCardBody>
                ...
                <>
                  <CDataTable
                    items={items}
                    fields={[
                      { key: 'name', label: 'Study Level', filter: false },
                      {
                        key: 'study_level_code',
                        label: 'Level Code',
                        filter: false,
                      },
                      {
                    ...
                    ccolumnFilterValue={columnFilterValue}
...
export default React.memo(StudyLevel)
