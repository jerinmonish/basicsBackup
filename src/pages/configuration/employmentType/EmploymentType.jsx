...
import { EmploymentList } from './../../../actions/configuration'
...
const EmploymentType = () => {
  ...
  const EmploymentData = useSelector((state) => state.configurationBackend)
  ...
  useEffect(() => {
    setLoading(true)
    dispatch(EmploymentList(params))
  }, [query])

  //useEffect to set Group list data and to update data for edit privielage
  useEffect(() => {
    if (EmploymentData?.employementListDetails?.data?.result.length > 0) {
      setItems(EmploymentData?.employementListDetails?.data?.result)
      setPages(EmploymentData?.employementListDetails?.data?.total_pages)
      setItemsPerPage(itemsPerPage)
      setLoading(false)
      setCtFrom(EmploymentData?.employementListDetails?.data?.from_count);
      setCtTo(EmploymentData?.employementListDetails?.data?.to_count);
      setTotalCnt(EmploymentData?.employementListDetails?.data?.total_count);
      // console.log(params)
    }
    else {
      setItems([])
  ...
  }, [EmploymentData?.employementListDetails?.data?.result])
...
export default React.memo(EmploymentType)
