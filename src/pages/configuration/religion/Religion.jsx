...
import { ReligionList } from './../../../actions/configuration'
...
const Religion = () => {
  ...
  const ReligionData = useSelector((state) => state.configurationBackend)
  ...
  useEffect(() => {
    setLoading(true)
    dispatch(ReligionList(params))
  }, [query])

  //useEffect to set Group list data and to update data for edit privielage
  useEffect(() => {
    if (ReligionData?.religionListDetails?.data?.result.length > 0) {
      setItems(ReligionData?.religionListDetails?.data?.result)
      setPages(ReligionData?.religionListDetails?.data?.total_pages)
      setItemsPerPage(itemsPerPage)
      setLoading(false)
      setCtFrom(ReligionData?.religionListDetails?.data?.from_count);
      setCtTo(ReligionData?.religionListDetails?.data?.to_count);
      setTotalCnt(ReligionData?.religionListDetails?.data?.total_count);
      // console.log(params);
    }
    else {
      setItems([])
              ...
              <CCardBody>
                ...
                <>
                  <CDataTable
                    items={items}
                    fields={[
                      { key: 'name', label: 'Religion', filter: false },
                      {
                        key: 'create_date',
                        label: 'Created At',
                        filter: false,
...
export default React.memo(Religion)
