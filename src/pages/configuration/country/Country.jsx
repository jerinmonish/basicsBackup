...
import { CountryList } from './../../../actions/configuration';
...
const Country = () => {
  ...
  const countryListData = useSelector(state => state.configurationBackend);
  ...
  useEffect(() => {
    setLoading(true);
    dispatch(CountryList(params));
  }, [query]);

  //useEffect to set Group list data and to update data for edit privielage
  useEffect(() => {
    if (countryListData?.countrylistDetails?.data?.result.length > 0) {
      setItems(countryListData?.countrylistDetails?.data?.result);
      setPages(countryListData?.countrylistDetails?.data?.total_pages);
      setItemsPerPage(itemsPerPage);
      setLoading(false);
      setCtFrom(countryListData?.countrylistDetails?.data?.from_count);
      setCtTo(countryListData?.countrylistDetails?.data?.to_count);
      setTotalCnt(countryListData?.countrylistDetails?.data?.total_count);
      // console.log(params);
    }
    else {
      setItems([])
  ...
  return (
    <main className="c-main">
      <ToastContainer />
      {
        ((params.tableFilterValue.length <= 0) && countryListData?.isLoading === true) ? <CLoader /> :
          <CFade>
            <CContainer fluid>
              <CCard>
                <CCardHeader>
                ...
                <CCardBody>
                  ...
                  {items.length > 0 &&
                    (<><CDataTable
                      items={items}
                      fields={[{ key: "name", label: "Country Name", filter: false },
                      { key: "code", label: "Country Code", filter: false },
