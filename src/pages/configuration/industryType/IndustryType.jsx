...
import { IndustryTypeList } from './../../../actions/configuration'
...
const IndustryType = () => {
  ...
  const IndustryTypeData = useSelector((state) => state.configurationBackend)
  ...
  useEffect(() => {
    setLoading(true)
    dispatch(IndustryTypeList(params))
  }, [query])

  //useEffect to set Group list data and to update data for edit privielage
  useEffect(() => {
    if (IndustryTypeData?.industrytypeListDetails?.data?.result.length > 0) {
      setItems(IndustryTypeData?.industrytypeListDetails?.data?.result)
      setPages(IndustryTypeData?.industrytypeListDetails?.data?.total_pages)
      setItemsPerPage(itemsPerPage)
      setLoading(false)
      setCtFrom(IndustryTypeData?.industrytypeListDetails?.data?.from_count);
      setCtTo(IndustryTypeData?.industrytypeListDetails?.data?.to_count);
      setTotalCnt(IndustryTypeData?.industrytypeListDetails?.data?.total_count);
  ...
  return (
    <main className="c-main">
      ...
      {params.tableFilterValue.length <= 0 &&
        IndustryTypeData?.isLoading === true ? (
        <CLoader />
      ) : (
        <CFade>
          <CContainer fluid>
              ...
              <CCardBody>
                ...
                <>
                  <CDataTable
                    items={items}
                    fields={[
                      { key: 'name', label: 'Industry Type', filter: false },
                      {
                        key: 'create_date',
                        label: 'Created At',
                        filter: false,
...
export default React.memo(IndustryType)
