...
import { RelationshipList } from './../../../actions/configuration';
...
const Relationship = () => {
  ...
  const RelationShipData = useSelector(state => state.configurationBackend);
  ...
  useEffect(() => {
    setLoading(true);
    dispatch(RelationshipList(params));
  }, [query]);

  //useEffect to set Group list data and to update data for edit privielage
  useEffect(() => {
    if (RelationShipData?.relationshipListDetails?.data?.result.length > 0) {
      setItems(RelationShipData?.relationshipListDetails?.data?.result);
      setPages(RelationShipData?.relationshipListDetails?.data?.total_pages);
      setItemsPerPage(itemsPerPage);
      setLoading(false);
      setCtFrom(RelationShipData?.relationshipListDetails?.data?.from_count);
      setCtTo(RelationShipData?.relationshipListDetails?.data?.to_count);
      setTotalCnt(RelationShipData?.relationshipListDetails?.data?.total_count);
      // console.log(params);
    }
    else {
      setItems([])
  ...
  }, [RelationShipData?.relationshipListDetails?.data?.result])
  ...
  return (
    <main className="c-main">
      <ToastContainer />
      {
        ((params.tableFilterValue.length <= 0) && RelationShipData?.isLoading === true) ? <CLoader /> :
          <CFade>
            <CContainer fluid>
              <CCard>
                <CCardHeader>
                ...
                <CCardBody>
                  ...
                  <><CDataTable
                    items={items}
                    fields={[{ key: "name", label: "Relationship", filter: false },
