...
import { DistrictList } from './../../../actions/configuration'
...
const District = () => {
  ...
  const DistrictData = useSelector((state) => state.configurationBackend)
  ...
  useEffect(() => {
    setLoading(true)
    dispatch(DistrictList(params))
  }, [query])

  //useEffect to set Group list data and to update data for edit privielage
  useEffect(() => {
    if (DistrictData?.districtListDetails?.data?.result.length > 0) {
      setItems(DistrictData?.districtListDetails?.data?.result)
      setPages(DistrictData?.districtListDetails?.data?.total_pages)
      setItemsPerPage(itemsPerPage)
      setLoading(false)
      setCtFrom(DistrictData?.districtListDetails?.data?.from_count);
      setCtTo(DistrictData?.districtListDetails?.data?.to_count);
      setTotalCnt(DistrictData?.districtListDetails?.data?.total_count);
      // console.log(params)
    }
                ...
                {items.length > 0 && (
                  <>
                    <CDataTable
                      items={items}
                      fields={[
                        { key: 'name', label: 'District', filter: false },
